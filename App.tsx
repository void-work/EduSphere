...
const App: React.FC = () => {
  ...
  const renderTool = () => {
    ...
    switch (activeTool) {
      ...
      default: return <Dashboard onSelectTool={setActiveTool} user={user} onCrash={triggerCrash} />;
    }
  };

  return (
    <div className="min-h-screen flex flex-col md:flex-row text-slate-900 selection:bg-indigo-500 selection:text-white">
      {/* Sidebar and Navigation as previously defined */}
      <aside className={`fixed inset-y-0 left-0 w-72 glass border-r border-slate-200/40 flex flex-col z-[45] md:static md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 flex items-center gap-4 mb-4">
          <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white"><Brain className="w-7 h-7" /></div>
          <span className="text-2xl font-black tracking-tighter">EduSphere</span>
        </div>
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
          <NavItem icon={<Sparkles size={18} />} label="Dashboard" active={activeTool === ToolType.DASHBOARD} onClick={() => { setActiveTool(ToolType.DASHBOARD); setIsMobileMenuOpen(false); }} />
          <NavItem icon={<BookOpen size={18} />} label="Textbook AI" active={activeTool === ToolType.TEXTBOOK} onClick={() => { setActiveTool(ToolType.TEXTBOOK); setIsMobileMenuOpen(false); }} />
          <NavItem icon={<Mic size={18} />} label="Live Tutor" active={activeTool === ToolType.TUTOR} onClick={() => { setActiveTool(ToolType.TUTOR); setIsMobileMenuOpen(false); }} />
          <NavItem icon={<Gamepad2 size={18} />} label="Kids Logic" active={activeTool === ToolType.KIDS} onClick={() => { setActiveTool(ToolType.KIDS); setIsMobileMenuOpen(false); }} />
        </nav>
        <div className="p-6">
          {user ? (
            <div className="flex items-center justify-between bg-slate-100/50 p-4 rounded-3xl border border-slate-200/50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white font-black">{user.name.charAt(0)}</div>
                <div><p className="text-sm font-black truncate max-w-[120px]">{user.name}</p><p className="text-[10px] text-slate-400 font-black">ACTIVE</p></div>
              </div>
              <button onClick={handleLogout} className="p-2 text-slate-400 hover:text-red-500"><LogOut size={16} /></button>
            </div>
          ) : (
            <button onClick={() => setIsLoginModalOpen(true)} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 transition-all">Sign In</button>
          )}
        </div>
      </aside>
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xl flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-12 border border-white">
            <h2 className="text-3xl font-black mb-8">Welcome Back</h2>
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <input name="firstName" required placeholder="First Name" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" />
                <input name="surname" required placeholder="Surname" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" />
              </div>
              <input name="email" type="email" required placeholder="Email Address" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" />
              <button type="submit" className="w-full py-6 bg-indigo-600 text-white rounded-[1.5rem] font-black uppercase tracking-widest shadow-2xl">Launch Experience</button>
            </form>
            <button onClick={() => setIsLoginModalOpen(false)} className="w-full mt-6 text-slate-400 font-black text-xs uppercase">Cancel Entry</button>
          </div>
        </div>
      )}
    </div>
...
const NavItem: React.FC<{ icon: React.ReactNode, label: string, active: boolean, onClick: () => void }> = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-5 py-4 rounded-[1.25rem] transition-all ${active ? 'bg-white text-indigo-600 shadow-xl border border-slate-100' : 'text-slate-400 hover:text-slate-900'}`}>
